(function () {
  ...
  // à l'intérieur. La classe html.stage est posée dès le <head> (build.py) pour éviter tout saut à l'affichage.
  ...
  function goTo(n) {
    ...
    slides.forEach(function (s, k) {
      ...
      s.classList.toggle('is-before', k < n);
      s.classList.toggle('is-after', k > n);
      s.classList.toggle('is-leaving', back && k === prev);   // en revenant, l'écran qui part redescend par-dessus
    });
    if (back) slides[n].scrollTop = slides[n].scrollHeight; else slides[n].scrollTop = 0;
  ...
  if (!stage) {
    ...
    if (wide && !reduced) window.addEventListener('load', function () {
      import('lenis').then(function (m) {   // hébergé par le site (assets/vendor/lenis.js, via la table d'import)
